...
argument-hint: <mot-clé ou slug d'un brief existant dans ~/vault/seo-globecreateur/briefs/>
---

Lis `docs/agent-seo/contexte.md` et `/ton-de-voix` (la voix d'Axel est non négociable). Cible : $ARGUMENTS

1. **Brief** : charge `~/vault/seo-globecreateur/briefs/<slug>.md` s'il existe, sinon exécute `/brief-contenu` d'abord
