      ...
      <Container>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center mb-32">
           <div className="space-y-8">
              <h1 className="text-sm font-black uppercase tracking-[0.2em] text-indigo-600 dark:text-indigo-400">Notre Mission</h1>
              <h2 className="text-4xl lg:text-7xl font-extrabold text-slate-900 dark:text-white leading-[1.1]">
           ...
           <div className="relative">
              <div className="rounded-[3rem] overflow-hidden shadow-2xl border border-slate-100 dark:border-slate-800 aspect-square">
                 <Image
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 mb-32">
           {stats.map((s, i) => (
              <div key={i} className="p-8 bg-slate-50 dark:bg-slate-900 rounded-3xl text-center space-y-2">
                 <p className="text-4xl lg:text-5xl font-black text-indigo-600 dark:text-indigo-400">{s.value}</p>
                 <p className="text-sm font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500">{s.label}</p>
              </div>
           ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-24 items-start mb-32">
           <div className="lg:col-span-5 space-y-6">
              <h2 className="text-4xl lg:text-5xl font-black text-slate-900 dark:text-white leading-tight">Pourquoi nous faisons ce que nous faisons.</h2>
              <div className="h-2 w-20 bg-indigo-600 dark:bg-indigo-500 rounded-full" />
           </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-6">
                 {[
                    ...
                    <div key={i} className="flex items-center gap-3">
                       ...
                       <span className="font-bold text-slate-900 dark:text-white" dangerouslySetInnerHTML={{ __html: item }} />
                    </div>
                 ))}
              </div>
           </div>
