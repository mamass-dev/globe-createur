...
export default async function ProjetPage({
  ...
  return (
    ...
    <section className="bg-[#0a0a0a] pt-12 lg:pt-20 pb-32">
      <Container>
        {/* HERO */}
        <header className="mb-20 space-y-10">
           <div className="flex flex-wrap items-center gap-4">
              <span className="px-4 py-1.5 bg-signal/15 text-signal text-xs font-black uppercase tracking-widest rounded-full">
                 Case Study
              </span>
              <span className="text-slate-400 text-sm font-bold uppercase tracking-widest">{fm.category}</span>
           </div>

           <h1 className="text-4xl lg:text-7xl font-extrabold text-ivory leading-tight tracking-tight max-w-4xl">
              {fm.title}
           </h1>

           <div className="grid grid-cols-2 md:grid-cols-4 gap-8 py-10 border-y border-[#1c1c1c]">
              <div>
                 <p className="text-xs font-black uppercase tracking-widest text-slate-400 mb-2">Client</p>
                 <p className="text-lg font-bold text-ivory">{fm.client || "Confidentiel"}</p>
              </div>
              <div>
                 <p className="text-xs font-black uppercase tracking-widest text-slate-400 mb-2">Ann&eacute;e</p>
                 <p className="text-lg font-bold text-ivory">{fm.year || "2025"}</p>
              </div>
              <div>
                 <p className="text-xs font-black uppercase tracking-widest text-slate-400 mb-2">Service</p>
                 <p className="text-lg font-bold text-ivory">{fm.category || "Digital Strategy"}</p>
              </div>
              <div>
                 <p className="text-xs font-black uppercase tracking-widest text-slate-400 mb-2">R&eacute;sultat</p>
                 <div className="flex items-center gap-2">
                    <div className="h-2 w-2 rounded-full bg-emerald-500" />
                    <p className="text-lg font-bold text-ivory">Succ&egrave;s</p>
                 </div>
              </div>
           </div>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-24 mb-32">
           <div className="lg:col-span-8 prose prose-indigo prose-xl max-w-none prose-h2:text-3xl prose-h2:font-black prose-h2:text-ivory prose-p:text-[#c7c4ba] prose-img:rounded-[2.5rem] prose-img:shadow-2xl">
              <MdxContent source={page.content} />
           </div>
           
           <div className="lg:col-span-4 space-y-8">
              <div className="p-8 bg-[#141414] rounded-sm border border-[#1c1c1c] space-y-8 sticky top-32">
                 <h3 className="text-xl font-bold text-ivory">Indicateurs de performance</h3>
                 <div className="space-y-6">
                    <div className="flex items-center gap-4">
                       <div className="h-10 w-10 rounded-none bg-[#1c1c1c] flex items-center justify-center text-signal">
                          <TrendingUp className="h-5 w-5" />
                       </div>
                       <div>
                          <p className="text-2xl font-black text-ivory">+45%</p>
                    ...
                    <div className="flex items-center gap-4">
                       ...
                       <div>
                          <p className="text-2xl font-black text-ivory">x2.5</p>
                    ...
                    <div className="flex items-center gap-4">
                       ...
                       <div>
                          <p className="text-2xl font-black text-ivory">-30%</p>
